...
class TextFieldEmbedder(torch.nn.Module):
    ...
    def get_output_dim(self):
        return sum(
            [embedder.get_output_dim() for embedder in self.token_embedders.values()]
        )
    ...
    def forward(self, text_field_input: dict[str, torch.Tensor], **kwargs):
        ...
        for k in sorted(self.token_embedders.keys()):
            ...
            if (
                self._embedder_to_indexer_map is not None
                and k in self._embedder_to_indexer_map
            ):
    ...
    def token_characters_embedder(cls, vocab, args):
        embedding = Embedding(
            vocab.get_vocab_size("token_characters"), embedding_dim=16
        )
