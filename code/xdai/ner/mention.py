    ...
    for i, v in enumerate(indices):
        if (i == 0) or (i == len(indices) - 1):
            ...
            if i % 2 == 0:
                ...
                if v + 1 < indices[i + 1]:
                    consecutive_indices.append(v)
    assert len(consecutive_indices) % 2 == 0 and len(consecutive_indices) <= len(
        indices
    )
    if len(indices) != len(consecutive_indices):
        logger.debug(
            ...
            % (
                ...
                " ".join([str(i) for i in consecutive_indices]),
            )
        )
...
class Mention(object):
    ...
    def indices(self):
        return sorted(
            [span.start for span in self.spans] + [span.end for span in self.spans]
        )
    ...
    def overlap_at_left(self):
        return (
            len(self._overlapping_spans) == 1 and list(self._overlapping_spans)[0] == 0
        )

    @property
    def overlap_at_right(self):
        return (
            len(self._overlapping_spans) == 1
            and list(self._overlapping_spans)[0] == len(self.spans) - 1
        )
        ...
        for i in range(len(mentions)):
            ...
            for j in range(len(mentions)):
                ...
                if Mention.contains(mentions[j], mentions[i]):
                    assert not Mention.contains(
                        mentions[j], mentions[i]
                    ), "TODO: multi-type mentions"
                    nested = True
                    break
            if not nested:
    ...
    def merge_overlapping_mentions(cls, mentions):
        ...
        while overlapping_may_exist:
            ...
            for i in range(len(mentions)):
                for j in range(len(mentions)):
                    ...
                    if Mention.overlap_spans(mentions[i], mentions[j]):
                        assert (
                            mentions[i].label == mentions[j].label
                        ), "TODO: two mentions of different types overlap"
                        overlapping_may_exist = True
                        merged_mention_start = min(mentions[i].start, mentions[j].start)
                        merged_mention_end = max(mentions[i].end, mentions[j].end)
                        merged_mention = Mention.create_mention(
                            [merged_mention_start, merged_mention_end],
                            mentions[i].label,
                        )
                        if (
                            merged_mention_start,
                            merged_mention_end,
                        ) not in merged_mentions:
                            merged_mentions[
                                (merged_mention_start, merged_mention_end)
                            ] = merged_mention
            ...
            mentions = [
                mention for mention in mentions if not mention.overlapping
            ] + list(merged_mentions.values())
        return mentions
