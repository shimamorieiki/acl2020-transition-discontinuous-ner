...
def parse_parameters(parser=None):
    ...
    ## Required
    parser.add_argument(
        "--input_ann", default="/data/dai031/Corpora/CADEC/cadec/original", type=str
    )
    parser.add_argument(
        "--input_text", default="/data/dai031/Corpora/CADEC/cadec/text", type=str
    )
    parser.add_argument(
        "--output_filepath", default="/data/dai031/Experiments/CADEC/all/ann", type=str
    )
        ...
        for document in os.listdir(args.input_ann):
            with open(os.path.join(args.input_ann, document), "r") as in_f:
                ...
                for line in in_f:
                    ...
                    sp = sp[1].split(" ")
                    label = sp[0]
                    if args.type_of_interest != "" and args.type_of_interest != label:
                        continue
                    ...
                    if mention != mention_from_text:
                        print(
                            "Update the mention from (%s) to (%s)."
